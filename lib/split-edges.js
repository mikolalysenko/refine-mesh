...
function splitEdges(mesh, splitBound) {
  ...
  var maxEdges = 3 * numCells // would be numVerts - numCells + g, for small g
                              // but don't know g so we use a conservative bound
  var ePtr = 0
  ...
  for(var i=0; i<numCells; ++i) {
    ...
    var ab2 = dist2(ax, ay, az, bx, by, bz)
    if(ab2 > splitBound) {
    ...
    var bc2 = dist2(bx, by, bz, cx, cy, cz)
    if(bc2 > splitBound) {
    ...
    var ca2 = dist2(cx, cy, cz, ax, ay, az)
    if(ca2 > splitBound) {
  ...
  for(var i=0; i<ePtr; i+=EDGE_SIZE) {
    ...
    if(a === pa && b === pb) {
      ...
      var nx = 0.5 * (normals[3*a] + normals[3*b])
      var ny = 0.5 * (normals[3*a] + normals[3*b])
      var nz = 0.5 * (normals[3*a] + normals[3*b])
      var nl = 1.0 / Math.sqrt(Math.pow(nx,2) + Math.pow(ny,2) + Math.pow(nz,2))
